...
print("🟢 Starting low‑stock alert script (TEST MODE - 10 products only)...", flush=True)
...
# ---------- 2. FETCH LOW STOCK QUANTS (exclude archived) ----------
print("📦 Fetching low‑stock quants...", flush=True)
domain = [
    ["location_id.usage", "=", "internal"],
    ["quantity", "<", 5],
    ["product_id.active", "=", True],   # exclude archived products
    ["product_id.type", "=", "product"]  # physical stockable only
]
...
<div style="background:#f9fafb; padding:40px 10px; font-family:Helvetica;">
  <div style="max-width:800px; margin:0 auto; background:#fff; border-radius:8px; border:1px solid #e5e7eb;">
    <div style="background:#111827; padding:25px 30px;">
      <h1 style="color:#fff; margin:0;">⚠️ TEST: Low Stock Alert (Top 10)</h1>
    ...
    <div style="padding:20px 30px; background:#fefefe; border-top:1px solid #eee; text-align:right;">
      <p style="margin:0; font-size:12px; color:#9ca3af;">Generated by GitHub Actions (TEST) • {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M UTC')}</p>
    </div>
  </div>
</div>
"""
...
# Retry logic for transient DNS/network issues
import time
