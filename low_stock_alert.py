...
print(f"📦 Found {len(quants)} low‑stock records. Unique products to process: {len(set(q['product_id'][0] for q in quants if q['product_id']))}", flush=True)
...
print("✅ Email sent successfully", flush=True)image_part.add_header("Content-ID", "<product_image>")
